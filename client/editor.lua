...
local function Editor_OnKeyRelease(key)
	...
	elseif (IsShiftPressed() and key == 'S' and EditorState == EDITOR_OPEN) then

		AddPlayerChat("[TEST] Pressed SHIFT + S")

		CallRemoteEvent('WorldSave')

	elseif (IsShiftPressed() and key == 'C' and EditorState == EDITOR_OPEN) then

		AddPlayerChat("[TEST] Pressed SHIFT + C")

		if (EditorSelectedObject == 0) then

			AddPlayerChat("[TEST] No object selected to be duplicated!")
		else

			-- Editor_SelectObject(0)
			AddPlayerChat("[TEST] Duplicated the object!")
...
local function Editor_OnRenderHUD()
	...
	if (EditorPendingPlacement and EditorState == EDITOR_OPEN) then
		...
		DrawText(w / 2, h / 2, "Click to spawn!")
...
function Editor_SelectObject(object)
	if ((IsShiftPressed() or EditorHighlightOverride) and IsValidObject(object)) then
...
local function OnScriptError(error)
	AddPlayerChat('<span color="#FF0000" style="bold">' .. error .. '</>')
end
